...
## 2026-09-23  NOTE P2-06 review (GPT-6 Sol) — verdict: NOT CLEAN; not merged
Reviewed Claude's pushed 41e70ee after trial merging it into origin/main cd374cf in isolated worktree rb/P2-06-review. REBUILD-LOG was the only conflict; both histories are present once in order. The requested source gates pass on that merged tree: footprint 10/0, chassis_spike 23/0 (all non-cost output identical to spike-P2-00.txt), suspension 12/0, surfaces 34/0, track_asset 23/0, road_tool 16/0, road_tool_v2 11/0, walls 8/0; all stderr empty. Spike flat footprint cost 135.3 us/tick and analytic crest with centre rays only 238.1 us/tick. TrackAsset TrackSurface cost 158.5 us/tick; the 150 -> 300 us gate follows section 6, and footprint's sharp-strip road cost was 162.3 us/tick (single ray 75.3). All ten legacy outputs matched docs/rebuild/baseline after CR/LF normalization; nine exited 0, dynamics Simcade exited 1 with the unchanged known braking failure. game.gd --check-only and headless editor startup passed; the editor class cache has RoadPath, RoadSection, WallPath and RoadScatter. Windowed --features passed 212/0. Every Godot invocation used Start-Process, WaitForExit(240000), kill-on-timeout, redirected stderr; no timeouts and every stderr was empty. gdformat --check -l 110 left the two new scripts unchanged.

Math review: minimizing ray distance d + rigid-tread drop(x,y) is the correct maximum-height envelope. The 1 mm bisection uses residuals from the centre plane and retains the nearest bearing top hit for a corner when intermediate mesh rays hit a face. FACE_COS filters only extra-ray candidates; the original centre hit keeps its single-ray normal at any angle. A missing centre is recast to the sample reach before edge detection. The smooth path returns the original centre hit with distance, point and normal bit-identical (1200/1200 sampled poses), adding only telemetry keys. The car call site preserves the centre ray and lower-deck recast, uses the wheel steering direction and body-frame tread side. Its compression-rate comment still says the edge normal leans back, although the chosen edge normal is the ground normal; correct that comment. footprint_rays counts only wheels returning a hit, despite its "queries made" description; clarify or count misses.

Additional P3-02b kerb drives used footprint.gd's road part at 80 km/h, with the sharp strip disabled to isolate the road kerbs, both sides set to each type, 0.6 m wide and 0.09 m high (RIBBED default ridges 8 mm/0.5 m). RIBBED: footprint 168.5 us/tick, peak 7.03x static; single ray 76.4 us/tick, peak 6.28x. SAUSAGE: footprint 182.8 us/tick, peak 4.48x; single ray 74.7 us/tick, peak 3.76x. Both stay under the section 6 mean cost budget, but both fail footprint.gd's "never harsher than the single ray" severity check; yaw rate also rises and SAUSAGE drops to one contacting wheel versus two. These are supplemental failures, not part of the original 10/0 gate.

Reproduced the ditch sweep at 76, 78, 80, 82 and 84 km/h crossed with steering gains 0.14, 0.15 and 0.16: single-ray maximum normal body depth 0.114-0.184 m, footprint 0.204-0.243 m, footprint deeper in 15/15 by 0.039-0.115 m (mean 0.079 m). At the footprint maximum each variant has one body contact and about 14-19 degrees of tilt; none sinks the CG below the surface. This is a systematic changed landing trajectory, not just a random pass/fail around 0.15 m. The model's shoulder/wall contact may physically cause it, but this review did not establish that; the 0.25 m gate is only 7 mm above the observed worst case. The old 0.15 m gate also failed 5/15 nearby single-ray variants, so its original margin was weak.

Fix before main: diagnose maximum-load ticks on RIBBED and SAUSAGE (ground normal, edge candidate, compression rate and contact point), then either correct the contact model or document a physically justified load/stability criterion and add both kerb types to a committed regression. Trace the ditch's one-corner landing torque/contact sequence against single ray and justify a robust body-depth bound. Correct the stale edge-normal and ray-count comments. New script UID sidecars were generated on the review branch. Main is unchanged.