...
`scripts/verification.gd` uses explicit failure collection rather than GDScript asserts, which may be removed in release exports. It covers acceleration; nonzero captured audio and mute/pause silence; generated sound streams; garage/settings; input mapping; five cameras; track validation and safe naming; dirty guards; JSON writes/replacement; ghosts/record identity; layout and all cars/tracks. Some domain operations are invoked directly; key remapping uses synthetic input and audio capture uses the engine mixer. This does not replace physical-device or human usability testing.
