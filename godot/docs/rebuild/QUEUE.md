...
| F-CI | CI tolerance: ci_gates.py lets legacy baselines differ by up to 5 % relative (and v2 JSON by 2 %) on Linux, far looser than needed (only legacy dynamics-simulation and showcase-laps differ there). Measure the real Linux-vs-Windows differences, scope the tolerance to those two suites at the smallest value that passes (print it), and remove the obsolete "spa roadster simulation" allowance (it passes since P4-07b) | Gemini | | open | read "REVIEW CI" |
...
| P4-vis | P4-04 visuals pose from Transform3D + per-wheel data, free attitude in flight; P4-05 cameras, instruments (minimap from the lap line, telemetry), audio surface ids, skid marks from contact_hits | Claude | P4-02 | open | P2-08's pose adapter is the start; owner moved it from Gemini to Claude (2026-09-23): no Gemini on P4 |
