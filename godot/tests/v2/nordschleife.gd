extends SceneTree
## P6-02b probe suite for Nürburgring Nordschleife: bakes the track in memory, validates the
...
## in tests/v2/laps.gd with the other circuits against the shared baseline.
...
const Generator = preload("res://trackgen/nordschleife.gd")
...
func _initialize() -> void:
	...
	if asset == null:
		push_error("Failed to build Nordschleife asset")
		quit(1)
		return
...
func _physics_process(_delta: float) -> bool:
	...
	if frames < 3:
		return false
	if ran:
		print("NORDSCHLEIFE RESULTS aborted by script error (see stderr)")
		quit(1)
		return true
	ran = true
	...
	print(
		"NORDSCHLEIFE RESULTS ", JSON.stringify({"checks": checks, "failures": failures, "results": results})
	)
